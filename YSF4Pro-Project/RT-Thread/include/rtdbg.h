...
#define _DBG_LOG_HDR(lvl_name, color_n)                    \
    rt_kprintf("\033["#color_n"m[%d/%d/%d %d:%d:%d "lvl_name"/%s %d] ",system_time.year, system_time.month, system_time.day,system_time.hour, system_time.min, system_time.sec, \
               FILE_NAME(__FILE__),__LINE__)
    //rt_kprintf("\033["#color_n"m["lvl_name"/"%s %d"] ",__FILE__, __LINE__)
...
#define dbg_hex(level, TAG, buf, size)                      \
    if ((level) <= DBG_LEVEL)                               \
    {                                                       \
        log_hexdump(TAG, buf, size);                        \
    }
...
#if (DBG_HEX >= DBG_LOG)
#define LOG_D(fmt, ...)      dbg_log(DBG_LOG, fmt, ##__VA_ARGS__)  //无颜色
#else
#define LOG_D(...)
#endif

#if (DBG_HEX >= DBG_INFO)
#define LOG_I(fmt, ...)      dbg_log(DBG_INFO, fmt, ##__VA_ARGS__)  //绿色
#else
#define LOG_I(...)
#endif

#if (DBG_HEX >= DBG_WARNING)
#define LOG_W(fmt, ...)      dbg_log(DBG_WARNING, fmt, ##__VA_ARGS__)  //黄色
#else
#define LOG_W(...)
#endif

#if (DBG_HEX >= DBG_ERROR)
#define LOG_E(fmt, ...)      dbg_log(DBG_ERROR, fmt, ##__VA_ARGS__)  //红色
#else
#define LOG_E(...)
#endif

#define log_printf(fmt, ...)  dbg_log(DBG_PRINTF, fmt, ##__VA_ARGS__)  //无颜色   
      
#if (DBG_HEX >= DBG_HEX)
#define LOG_HEX(TAG, buf, size)       dbg_hex(DBG_HEX, TAG, buf, size)
#else
#define LOG_HEX(...)
#endif
