...
{
    static uint8_t ice_data[125] = {0};
    
    //DT45IceData_t *data = (DT45IceData_t *)data_buff;
    
    memset(ice_data, 0, sizeof(ice_data));
    memcpy(ice_data, data_buff, data_len);
    int TensionOrignal = ((int)ice_data[3] << 24) | ((int)ice_data[4] << 16) | ((int)ice_data[5] << 8) | (int)ice_data[6];
    
    float TensionFloatKG = (float)TensionOrignal / 100.0;
    
    if (ice_ID == 0x01){
        DeviceBatteryParam.Battery_Voltage = TensionFloatKG;
    }else{
        DeviceBatteryParam.Battery_Current = TensionFloatKG;
    }
    
    LOG_D("ID: %d, Tension:%f",ice_ID,TensionFloatKG);
}
...
void DS_DeviceIce_Run()
{
    ...
    rt_time_t NowTime, LastTime=DS_SysTime;
    ...
    while(1)
    {
        ...
        if (CollectDataCnt < 3 && (NowTime - LastTime) > 60) {
            ...
            for (int j = 0; j < 2; j++) {
                ...
                LastTime = NowTime;
                AutoCollectIceData(DeviceIceModuleCfg.ID[j]);
