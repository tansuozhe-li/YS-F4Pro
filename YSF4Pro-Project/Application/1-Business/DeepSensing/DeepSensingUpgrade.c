...
*  文件名称：  BusinessDeepSensingUpgrade.c
*  内容摘要：  DS模块初始化
*  其他说明：
*  当前版本：  EIT-L_V1.0.0
*  作    者：  
...
static DSUpgradeStateParam_t DSUpgradeStateParam;
...
BOOL DS_MsgProc_UpdateFrameDataResp(GW2015FrameHeader_t *pFrame)
{
    ...
    if (!DS_OTAUpgradeDataWriteToFlash(pData)){
        ...
        LOG_I("DXFF Update Request Frame index : %d Success", pData->File_Frame_Index);
    }
    rt_sem_release(update_sem_data);
    return 0;
}
...
BOOL DS_CheckFlashUpdateState()
{
    ...
    if (OTAUpgradeFileHead.File_Type == OTA_UPGRADE_FILE_TYPE_APP) { //app应用程序
        ...
        //判断文件头是否相同，如相同则不需要升级
        AssertErrorNoPrint(memcmp(&OTAUpgradeFileHead, &OnchipUpgradeFileHead, sizeof(OTAUpgradeFileHead)) != 0, return FALSE);
    ...
    for (int i = 0; i < OTAUpgradeFileHead.File_Frame_Num; i++) {
        if (UpFrameState[i].upgrade_frame_state == UPGRADE_DATA_OK) {
            ...
            LOG_I("上一次升级未完成，当前进度 ： %d / %d 继续升级", request_upgrade_frame_index, OTAUpgradeFileHead.File_Frame_Num);
            return TRUE; //需继续升级
        }
    }
    //LOG_I("不需要继续升级");
...
static BOOL DS_UpdateProgramOnchipFlashErase(DSOTAUpgradeInform_t *file_head)
{
    ...
    if (file_head->File_Type == OTA_UPGRADE_FILE_TYPE_APP) {
        if (OnChipFlashEraseSector(UPDATE_APP_PKG_HEAD_BEGIN, 5) != 1) {
            return FALSE;
        }
        ...
        if (OnChipFlashEraseSector(UPDATE_APP_ADDR_BEGIN, sector_num) != 1) {
            return FALSE;
        }
...
void OnchipFileCheckToLoad()
{
    if (DS_UpdateOnchipFileCheck()) { //升级失败，校验片内程序
        ...
        LOG_E("设备重启");
        EITL_REBOOT(); //return FALSE 校验失败，设备复位
    }
}
...
{   
    ...
    while(1)
    {
        ...
        //等待心跳包成功接收  
        AssertContinueNoPrint(Recv_Heartbeat_flag == GW2015DataStateOK, {});
        
        //建立通信后，读取保存到片外Flash中的升级状态，判断是否需要继续升级
        if (load_delay == 0) {
            if (DS_CheckFlashUpdateState()){
                DSUpgradeStateParam.upgrade_state = UPDATE_STATE_INFORM;
            }    
        }
        load_delay = 1;
        SetTaskStatus(BUSINESS_DS_UPDATE_TASK_PRIO, TASK_IDLE);        
        //等待升级通知
        ...
        SetTaskStatus(BUSINESS_DS_UPDATE_TASK_PRIO, TASK_WORKING);
        //接受到升级通知，进入升级传输过程
        ...
        for (; request_upgrade_frame_index < DSUpgradeStateParam.file_frame_num;) {
            ...
            rt_sem_take(update_sem_data,9000 + delay_cnt*1000);
            if (UpFrameStateTable[request_upgrade_frame_index].upgrade_frame_state == UPGRADE_DATA_OK) {
                ...
                delay_cnt++;
                LOG_I("DXFF Update Request Frame index : %d / %d", request_upgrade_frame_index, DSUpgradeStateParam.file_frame_num);
        ...
        if ((update_state == UPDATE_STATE_FINISH) && DS_UpdateFinishCheckFileState()) {
            ...
            DSUpgradeStateParam.upgrade_state = UPDATE_STATE_ERROR;
        }
    }      
}
