 ...
 */
void log_hexdump(const char *tag, const rt_uint8_t *buf, rt_size_t size)
{
    rt_size_t i;
    rt_kprintf("%s: , size: %d",tag, size);
    rt_kprintf("\n");
...
void log_hexdump16(const char *tag, const rt_uint8_t *buf, rt_size_t size)
{
    int i = 0;
    rt_kprintf("%s: , size: %d",tag, size);
    for(; i<size; i++){
