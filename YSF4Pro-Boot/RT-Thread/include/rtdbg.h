...
#ifndef DBG_SECTION_NAME
#define DBG_SECTION_NAME    "EITL"
#endif
#endif /* DBG_TAG */

#ifdef DBG_ENABLE

#define DBG_LVL  DBG_HEX
#ifdef DBG_LVL
#ifndef DBG_LEVEL
...
#if (DBG_LEVEL >= DBG_LOG)
#define LOG_D(fmt, ...)      dbg_log_line("D", 0, fmt, ##__VA_ARGS__)  //无颜色
#else
#define LOG_D(...)
#endif

#if (DBG_LEVEL >= DBG_INFO)
#define LOG_I(fmt, ...)      dbg_log_line("I", 32, fmt, ##__VA_ARGS__)  //绿色
#else
#define LOG_I(...)
#endif

#if (DBG_LEVEL >= DBG_WARNING)
#define LOG_W(fmt, ...)      dbg_log_line("W", 33, fmt, ##__VA_ARGS__)  //黄色
#else
#define LOG_W(...)
#endif

#if (DBG_LEVEL >= DBG_ERROR)
#define LOG_E(fmt, ...)      dbg_log_line("E", 31, fmt, ##__VA_ARGS__)  //红色
#else
#define LOG_E(...)
#endif
...
#if (DBG_LEVEL >= DBG_HEX)
#define LOG_HEX(TAG, buf, size)       log_hexdump(TAG, buf, size)
#define LOG_HEX16(TAG, buf, size)     log_hexdump16(TAG, buf, size)
#else
#define LOG_HEX(...)
#endif

#define log_printf(fmt, ...)     rt_kprintf(fmt, ##__VA_ARGS__)
